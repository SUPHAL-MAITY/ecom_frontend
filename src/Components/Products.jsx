...
const Products = forwardRef(({ priceMin, priceMax, gender }, ref) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${url}/api/v1/get-all-products?page=${page}`,
  ...
  const fetchFilteredData = async () => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.get(
        `${url}/api/v1/filter?priceMax=${priceMax}&priceMin=${priceMin}&${gender
          .map((c) => `gender=${c}`)
          .join("&")}`
      );
    ...
    <>
      ...
      {!loading && (
         ...
         <div className="mx-auto max-w-2xl px-4  sm:px-6 sm:py-4 lg:max-w-7xl lg:px-8">
           <div className="mt-[2px] grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-2 2xl:grid-cols-3 xl:gap-x-8">
